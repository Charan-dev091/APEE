"""
APEE — Hyperliquid + Claude Advanced Pipeline  ⚠ EXPERIMENTAL / INCOMPLETE ⚠
===============================================================================
STATUS: This entry point cannot run as-is. It depends on several modules
that have not yet been committed to the repository:

  MISSING MODULES (import will fail):
    src/risk_manager.py          (RiskManager class)
    src/trading/hyperliquid_api.py (HyperliquidAPI class)
    src/config_loader.py         (CONFIG dict loader)
    src/utils/formatting.py      (format_number, format_size)
    src/utils/prompt_utils.py    (json_default, round_or_none, round_series)

  EXTRA REQUIREMENTS (not in core requirements.txt):
    anthropic>=0.25.0            (Strategist + Orchestrator)
    aiohttp                      (API server)
    numpy                        (Analyst LightGBM inference)

DIFFERENCES from root main.py (the canonical entry point):
  - Targets Hyperliquid perpetuals exchange (crypto only)
  - Uses Anthropic Claude for Strategist (structural analysis)
  - Uses LightGBM for Analyst (fast ML signals)
  - Has live TP/SL order management and fill reconciliation
  - No e-commerce mode
  - More advanced risk management (RiskManager, Sharpe tracking)

TO USE THIS FILE:
  1. Implement the missing modules listed above
  2. pip install anthropic aiohttp numpy lightgbm
  3. Configure ANTHROPIC_API_KEY and Hyperliquid credentials in .env
  4. python src/main.py --assets BTC ETH --interval 5m

For the working system, run the canonical entry point instead:
  python main.py
"""
...
from src.agents.orchestrator import APEEOrchestrator as TradingAgent
