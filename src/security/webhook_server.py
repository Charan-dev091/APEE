...
async def handle_request(reader, writer, mandate_manager, ws_broadcaster):
    ...
    try:
        ...
        if method == "GET" and path == "/health":
            ...
            # Signature from frontend (real WebAuthn assertion in production).
            # If absent, generate the expected HMAC so auto-approve PoC path works.
            signature   = data.get("signature", "")
            if not signature:
                # Look up the challenge from pending mandates and sign it.
                pending = mandate_manager.get_pending()
                challenge = next(
                    (m["challenge"].replace("...", "") for m in pending if m["mandate_id"] == mandate_id),
                    ""
                )
                # get_pending() truncates the challenge for UI display — fetch full
                # challenge via the mandate directly (it's stored un-truncated in memory)
                mandate_obj = mandate_manager._pending.get(mandate_id)
                if mandate_obj:
                    signature = _make_expected_sig(mandate_obj.challenge)

            result = await mandate_manager.approve(mandate_id, signature)

            if result.get("success"):
                # Notify frontend via WebSocket
                ws_broadcaster.push("MANDATE_APPROVED", {
            ...
            ws_broadcaster.push("MANDATE_REJECTED", {
                "mandate_id": mandate_id,
                "reason":     reason,
            })
            response_body = json.dumps({"success": True})
